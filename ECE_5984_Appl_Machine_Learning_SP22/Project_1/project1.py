...
for i in range(3,9):
    ...
    training_score = clf_entropy.score(X_train_flag, y_train_flag['NEXTDAYPRECIPFLAG'])
    true_positive_entropy, matrix_df_entropy = get_true_positive(clf_entropy, X_test_flag, y_test_flag)
    
    score_df.loc[idx,'Tree Depth'] = i
    score_df.loc[idx,'Entropy Training Score'] = training_score
    score_df.loc[idx,'Entropy Test Score'] = true_positive_entropy
...
for i in range(3,9):
    ...
    training_score = clf_gini.score(X_train_flag, y_train_flag['NEXTDAYPRECIPFLAG'])
    true_positive_gini, matrix_df_gini = get_true_positive(clf_gini, X_test_flag, y_test_flag)
    
    score_df.loc[idx,'Gini Training Score'] = training_score
    score_df.loc[idx,'Gini Test Score'] = true_positive_gini
...
mean_squared_error(y_test_amt, lin_model_pred_test)
...
mean_squared_error(y_test_amt, ridge_model_pred_test)
...
plt.scatter(X_test_amt, y_test_amt, color = 'black')
plt.plot(X_test_amt, lin_model_pred_test, color="blue", linewidth=3)
